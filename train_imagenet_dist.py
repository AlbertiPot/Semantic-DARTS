...
parser.add_argument('--lr_scheduler',
                    type=str,
                    default='linear',
                    help='lr scheduler, linear or cosine')
...
args.save = 'results/eval-{}-{}'.format(args.save,
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.local_rank == 0:
            valid_acc_top1, valid_acc_top5, valid_obj = infer(
                val_loader, model.module, criterion)
    ...
    logging.info('best_acc_top1: {}, best_acc_top5: {}'.format(
        best_acc_top1, best_acc_top5))
...
def adjust_lr(optimizer, epoch):
    ...
    if args.epochs - epoch > 5:
        ...
        lr = args.learning_rate * (args.epochs - epoch) / (
            (args.epochs - 5) * 5)
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
    return lr
...
def train(train_loader, model, criterion, optimizer):
    ...
    for i, (image, target) in enumerate(train_loader):
        ...
        n = image.size(0)
        objs.update(loss_reduce.item(), n)
        ...
        if i % args.report_freq == 0 and args.local_rank == 0:
            logging.info(
                ...
                i, train_iters, objs.avg, top1.avg, top5.avg, batch_time.avg,
                float(datatime))
...
def infer(val_loader, model, criterion):
    ...
    for i, (image, target) in enumerate(val_loader):
        ...
        if i % args.report_freq == 0:
            logging.info(
                'VALID Step: %03d/%03d Objs: %e R1: %f R5: %f Datatime: %.3f',
                i, val_iters * args.world_size, objs.avg, top1.avg, top5.avg,
                float(datatime))
